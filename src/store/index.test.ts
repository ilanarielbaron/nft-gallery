...
import nftsReducer, { disconnect, fetchNFTs, initialState as initialNFTsState, toggleLoading, toggleNFTLike } from "./nftsReducer";
...
describe("tests for WalletReducer", () => {
  ...
  it('connect a wallet', () => {
    ...
    expect(walletReducer({ wallet: previousState }, connectWallet({ address: 'address1', chainId: '0x1' }))).toEqual(
      { wallet: { isConnected: true, address: 'address1', chainId: '0x1' } }
    );
  });

  it('disconnect a wallet', () => {
    expect(walletReducer({ wallet: { isConnected: true, address: 'address1', chainId: '0x1' } },
      disconnectWallet())).toEqual({ wallet: { isConnected: false } })
  });
});
...
describe("tests for NFTsReducer", () => {
  ...
  it('fetchNFTs', () => {
    expect(nftsReducer({ nfts: [], isLoading: false }, fetchNFTs([mockNFT]))).toEqual(
      { nfts: [mockNFT], isLoading: false }
    );
  });

  it('toggleNFTLike', () => {
    expect(nftsReducer({ nfts: [mockNFT], isLoading: false }, toggleNFTLike({ id: 'mockId' }))).toEqual(
      { nfts: [{ ...mockNFT, isLiked: true }], isLoading: false }
    );
  });
});
